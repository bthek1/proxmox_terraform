...
- ⚠️ **Host storage: the `pve/data` thin pool filled to 100% on 2026-08-02** and silently corrupted guest filesystems for days (VM 109 unbootable, CTs failing to mount). Nothing trims automatically and the VG has no headroom to autoextend. **Resolved 2026-08-04:** the added 4 TB NVMe (`nvme1n1`) is now LVM-thin storage **`nvme4tb-lvm`** (VG `nvme4tb`, thinpool `data`) and **all 12 guests were migrated onto it** — `pve/data` is now ~empty (0.01%) and the old boot-pool-full risk is retired. The **same no-auto-trim gotcha applies to the new pool**, so when touching the node glance at `lvs nvme4tb/data` (that's where the guests live now); **Data% ≥ 95% is an emergency** (recover with `pct fstrim` per CT / `qm guest exec 109 -- fstrim -av`). Note: the two **online-moved** VM disks (109 `scsi0`, 204 `scsi0`) sit thick at Data% 100% on the new pool — in-guest `fstrim` will **not** re-thin them (only an offline `qm disk move` round-trip does); harmless at ~22% pool use. If sudo/DNS/binaries inside VM 109 ever break mysteriously, suspect zero-filled files from an EIO window — `qm guest exec 109` on the host is a root channel that bypasses broken guest sudo. Full playbook: [docs/Incident/proxmox-thinpool-full-incident.md](docs/Incident/proxmox-thinpool-full-incident.md)
