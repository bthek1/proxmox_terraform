...
- **This workstation is VM 109 ("Main") on the Proxmox host.** All peripherals (GPU incl. HDMI audio, USB controller, Bluetooth, webcams) reach it via passthrough — see [docs/109_PERIPHERAL_PASSTHROUGH.md](docs/109_PERIPHERAL_PASSTHROUGH.md). `hostpci`/`usb` config changes need a host-side `qm reboot 109` (guest reboot does not apply them), and rebooting VM 109 kills any session running here
