...
Full map of how the Proxmox host's peripherals reach VM 109 ("Main" — this workstation) and the LXC containers. Verified live against `qm config 109`, `lspci`/`lsusb` on host and guest on **2026-07-25** (webcam endgame: MX Brio refunded → interim C922 → **Brio 500 as sole VM camera**).
...
| AMD HD Audio controller (`1022:15e3`) | PCI `11:00.6` (was `10:00.6` until the 4 TB NVMe shifted the bus, 2026-08-04) | PCIe passthrough | `hostpci1` | `02:00.0` (PipeWire analog-stereo sink) |
| **Keyboard** — Kensington 2.4G dongle | `047d:8188` | Per-device USB redirection (QEMU emulated) | `usb2` | emulated bus |
| Webcam — **Logitech Brio 500** (added 2026-07-24 evening; sole VM camera since 2026-07-25) | `046d:0943` | Per-device USB redirection. Native USB 2.0 over USB-C (1080p30 MJPEG, stereo mic @48 kHz). **Verified: sustained 29.8 fps at 1080p even in dim light** (RightLight 4) **and mic CLEAN under sustained 1080p load** (jumps ≤1.7k, peaks ≤−18 dB). Auto-framing/Show Mode need Logi software (Win/Mac) — plain UVC here. Use MJPEG in OBS, never YUYV above ~480p | `usb5` | emulated bus, `/dev/video2`/`video3` (indices may shift to 0/1 after a guest reboot — re-pick in OBS if the source breaks) |

> **Logitech C922** (`046d:085c`): removed from VM 109 and unplugged 2026-07-25 — it was redundant next to the Brio 500, and two cameras can't stream 1080p simultaneously on the shared emulated bus (green frames; see Gotchas). It remains a known-good USB2 spare; to re-add: `sudo qm set 109 -usb3 host=046d:085c` (or give it to an LXC via device nodes).
...
| ~~UGREEN 2K webcam~~ → **moved to LXC 205** (2026-07-24 evening), video + mic | `1bcf:2284` | Host owns it (`uvcvideo` + `snd-usb-audio`, host sound card 0 "U2K"); device-passed into **LXC 205 (knowledge-lab)**: video `/dev/video0`/`video1` as `dev5`/`dev6`, mic as `dev7` (`/dev/snd/controlC0`), `dev8` (`/dev/snd/pcmC0D0c`), `dev9` (`/dev/snd/timer`) — all `mode=0666`, hotplugged into the running CT, video + ALSA capture (`hw:0,0`) both verified. VM 109's `usb5` entry deleted. ⚠️ Host video indices AND sound-card numbers can renumber if devices are re-plugged or the host reboots — re-check all five `dev` paths then | `pct` `dev5`–`dev9` on CT 205 | `/dev/video0`/`video1` + `/dev/snd/*` in CT 205 |
...
- ⛔ **The Raphael CPU xHCI controllers (IDs `1022:15b6`/`15b7`; at `11:00.3`/`11:00.4` since 2026-08-04, previously `10:00.3`/`10:00.4` — identify by ID, addresses shift when drives are added) must NEVER be passed to a guest — tried 2026-07-24, instant HOST hard-reset.** The earlier plan here ("alone in IOMMU group, working `pm` reset → ideal candidates") was wrong: the advertised reset is broken in hardware, and `hostpci2: 0000:10:00.4` + `qm reboot 109` power-cycled the whole host with no log trace, then **crash-looped the host on every boot** while `startall` retried VM 109 (`onboot`). Reverted same day (`--delete hostpci2`, `usb3` forward restored). The native-SuperSpeed path for the Brio is fully exhausted (chipset `0c:00.0` reset-loops the Brio; CPU xHCIs kill the host) — 1080p30 MJPEG over USB 2.0 redirection is permanent. Full write-up: [incident doc, Mode E](Incident/proxmox-vm109-gpu-freeze-incident.md#update--2026-07-24-mode-e-cpu-xhci-10004-passthrough--instant-host-hard-reset--boot-crash-loop--reverted-do-not-retry).
- **UGREEN cam is a native USB2 device** — 480M is its ceiling; MJPEG 2K fine.
- **Only ONE camera may stream 1080p at a time in VM 109** (verified 2026-07-25): all forwarded USB shares one emulated 480M controller, and OBS with both the C922 and Brio 500 sources active truncates MJPEG frames (**green image**) and starves YUYV (**flicker**) — kernel logs xHCI ring-error bursts. One camera + its mic at 1080p30 MJPEG is clean; two cameras need both dropped to 720p30 MJPEG. YUYV above ~480p is never viable here (raw 1080p ≈ 1 Gbps > the whole bus). OBS counts a source as active when visible in program OR studio-mode preview.
